...
public abstract class ConfigFile {
    ...
    public ConfigFile(String name, HCF plugin, boolean feature) {
        name += (name.endsWith(".yml") ? "" : ".yml");

        file = new File(plugin.getDataFolder() + (feature ? (File.separator + "feature") : "") + File.separator + name);
        if(!file.exists()) {
            try {
    ...
    public ConfigFile(String name) {
        this(name, HCF.getInstance(), false);
    }
